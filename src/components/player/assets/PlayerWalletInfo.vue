<template>
  <div>
    ...
    <el-card>
      <el-button type="primary" @click="addBalance(100)">充值100元</el-button>
    </el-card>
...
export default {
  ...
  created() {
    this.playerWalletLoad();
  },
  methods:{
    async playerWalletLoad(){//加载开发商钱包信息
      const {data: res} = await this.$axios.get('/apis/player/getPlayerWallet');
      this.userWallet=res.data;

    },
    async addBalance(x){
      const {data: res} = await this.$axios.put('/apis/player/addBalance/'+x);
